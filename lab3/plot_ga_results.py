...
def plot_history(history_file: str, output_file: str):
    ...
    plt.plot(history["generation"], history["bestCost"])
    plt.xlabel("Pokolenie")
    plt.ylabel("Najlepszy koszt")
    plt.title("Algorytm genetyczny - zmiana najlepszego kosztu w czasie")
    plt.grid(True)
    plt.tight_layout()
...
def plot_runs_bar(runs_file: str, output_file: str):
    ...
    plt.ylabel("Najlepszy koszt")
    plt.title("Algorytm genetyczny - wynik w kolejnych próbach")
    plt.legend()
...
def plot_runs_histogram(runs_file: str, output_file: str):
    ...
    plt.title("Algorytm genetyczny - rozkład wyników")
    plt.legend()
    plt.grid(True, axis="y")
    plt.tight_layout()
    plt.savefig(output_file, dpi=300)
...
def plot_best_route(route_file: str, output_file: str, invert_y: bool):
    ...
    plt.plot(route["y"], route["x"], marker="o", markersize=2, linewidth=0.7)

    if len(route) <= 200:
        for _, row in route.iterrows():
            plt.text(row["y"], row["x"], str(int(row["id"])), fontsize=7)

    plt.xlabel("X")
    plt.ylabel("Y")
    plt.title("Algorytm genetyczny - najlepsza znaleziona trasa")
    plt.grid(True)
    plt.axis("equal")
...
def save_summary(runs_file: str, output_file: str):
    ...
    text = []
    text.append("Podsumowanie wyników algorytmu genetycznego")
    text.append("=" * 50)
    ...
    text.append(f"Pokolenia najlepszej próby: {int(best_row['generationsDone'])}")
...
def main():
    ...
    parser.add_argument(
        "--name",
        default="",
        help="Suffix dodawany do nazw plików wynikowych, np. eg7146_ox",
    )
    ...
    for file in [args.history, args.runs, args.route]:
        if not Path(file).exists():
            raise FileNotFoundError(f"Nie znaleziono pliku: {file}")

    suffix = args.name.strip()

    history_out = with_suffix("ga_history", suffix, "png")
    bar_out = with_suffix("ga_runs_bar", suffix, "png")
    hist_out = with_suffix("ga_runs_histogram", suffix, "png")
    route_out = with_suffix("ga_best_route", suffix, "png")
    summary_out = with_suffix("ga_summary", suffix, "txt")
